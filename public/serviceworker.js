...
self.addEventListener('fetch', (event) => {
    event.respondWith(
        caches.match(event.request) // matchar alla requests som sidan får, api-anrop
        .then(() => {
...
self.addEventListener('activate', (event) => {
    ...
    event.waitUntil(
        ...
        .then((cacheNames) => {
            Promise.all(
                cacheNames.map((cacheName) => { // loopar igenom cacheNames och kontroller om cacheWhiteList inte innehåller cachename 
